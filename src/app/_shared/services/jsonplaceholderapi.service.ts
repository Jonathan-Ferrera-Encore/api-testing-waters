...
export class JsonPlaceholderApiService {
    ...
    getPosts(): Observable<JsonPlaceholderPost[]> {
        ...
        return this.http.get<JsonPlaceholderPost[]>(this.baseUrl + '/posts').pipe(takeUntil(destruct), take(1));
    }

    getPostsById(id: number): Observable<JsonPlaceholderPost> {
        const destruct: Observable<0> = timer(5000);
        return this.http.get<JsonPlaceholderPost>(this.baseUrl + '/posts/' + id).pipe(takeUntil(destruct), take(1));
    }

    postPosts(jsonPlaceholderPost: JsonPlaceholderPost): Observable<JsonPlaceholderPost> {
        const destruct: Observable<0> = timer(5000);
        return this.http.post<JsonPlaceholderPost>(this.baseUrl + '/posts', jsonPlaceholderPost).pipe(takeUntil(destruct), take(1));
    }
}
